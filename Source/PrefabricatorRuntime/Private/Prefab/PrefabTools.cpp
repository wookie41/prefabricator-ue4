...
void GatherParentActorsMap(TArray<AActor*> InActors, TMap<AActor*, AActor*>& OutParentMap)
{
    for (AActor* Actor : InActors)
    {
        AActor* ParentActor = Actor->GetAttachParentActor();
        // Skip parents outside of the selection,
        // those will be simply attached to the prefab actor itself
        if (InActors.Contains(ParentActor))
        {
            OutParentMap.Add(Actor, ParentActor);
        }
        else
        {
            OutParentMap.Add(Actor, nullptr);
        }

        TArray<AActor*> AttachedActors;
        Actor->GetAttachedActors(AttachedActors, false);
        GatherParentActorsMap(AttachedActors, OutParentMap);
    }
}

void GatherFlattenedHierarchy(AActor* InActor, TArray<AActor*>& OutHierarchy, bool bSkipUs)
{
    if (!bSkipUs)
    {
        OutHierarchy.Add(InActor);
    }
    TArray<AActor*> AttachedActors;
...
int32 FPrefabTools::GetRandomSeed(const FRandomStream& InRandom) { return InRandom.RandRange(0, 10000000); }
...
bool FPrefabTools::CanCreatePrefab() { return GetNumSelectedActors() > 0; }
...
APrefabActor* FPrefabTools::CreatePrefabFromActors(const TArray<AActor*>& InActors)
{
    ...
    TMap<AActor*, AActor*> ParentActorsMap;
    GatherParentActorsMap(InActors, ParentActorsMap);
    ...
    // Restore the parents
    for (auto& entry : ParentActorsMap)
    {
        if (entry.Value)
        {
            entry.Key->DetachFromActor(FDetachmentTransformRules::KeepWorldTransform);
            entry.Key->AttachToActor(entry.Value, FAttachmentTransformRules::KeepWorldTransform);
        }
    }
...
void FPrefabTools::AssignAssetUserData(AActor* InActor, const FGuid& InItemID, APrefabActor* Prefab)
{
    ...
    UPrefabricatorAssetUserData* PrefabUserData =
      RootComponent->GetAssetUserData<UPrefabricatorAssetUserData>();
    if (!PrefabUserData)
    {
...
void FPrefabTools::SavePrefabToAsset(APrefabActor* PrefabActor)
{
    TArray<AActor*> AttachedActors;
    GatherFlattenedHierarchy(PrefabActor, AttachedActors, true);

    TMap<AActor*, AActor*> ParentActorsMap;
    GatherParentActorsMap(AttachedActors, ParentActorsMap);
    ...
    for (AActor* Actor : AttachedActors)
    {
        ...
        if (AActor* ParentActor = ParentActorsMap[Actor])
        {
            Actor->AttachToActor(ParentActorsMap[Actor], FAttachmentTransformRules::KeepWorldTransform);
        }
        else
        {
            Actor->AttachToActor(PrefabActor, FAttachmentTransformRules::KeepWorldTransform);
        }
    }
}
...
void FPrefabTools::SaveStateToPrefabAsset(APrefabActor* PrefabActor)
{
    ...
    UPrefabricatorAsset* PrefabAsset = PrefabAsset =
      Cast<UPrefabricatorAsset>(PrefabActor->PrefabComponent->PrefabAssetInterface.LoadSynchronous());
    if (!PrefabAsset)
    {
    ...
    // Make sure the children do not have duplicate asset user data template ids
    {
        ...
        for (AActor* ChildActor : Children)
        {
            ...
            {
                UPrefabricatorAssetUserData* ChildUserData =
                  ChildActor->GetRootComponent()->GetAssetUserData<UPrefabricatorAssetUserData>();
                if (ChildUserData)
                {
    ...
    for (int i = 0; i < ItemsToSave.Num(); ++i)
    {
        const FSaveContext& SaveInfo = ItemsToSave[i];
        ...
        if (ChildActor && ChildActor->GetRootComponent())
        {
            ...
            if (PrefabActor->FlattenedParentIndices.Num() > 0)
            {
                ActorData.ParentIndex = PrefabActor->FlattenedParentIndices[i];
            }
...
{
    bool
    GetPropertyData(const FProperty* Property, UObject* Obj, UObject* ObjTemplate, FString& OutPropertyData)
    {
        if (!Obj || !Property)
            return false;
        ...
        Property->ExportTextItem(OutPropertyData,
    ...
    bool HasDefaultValue(UObject* InContainer, UObject* InDiff, const FString& InPropertyPath)
    {
        ...
        {
            UE_LOG(LogPrefabTools,
                   Log,
                   TEXT("Property differs: %s\n> %s\n> %s"),
                   *InPropertyPath,
                   *PropertyValue,
                   *DefaultValue);
        }
    ...
    bool
    ShouldSkipSerialization(const FProperty* Property, UObject* ObjToSerialize, APrefabActor* PrefabActor)
    {
    ...
    {
        if (!InObjToDeserialize)
            return;

        for (UPrefabricatorProperty* PrefabProperty : InProperties)
        {
            if (!PrefabProperty || PrefabProperty->bIsCrossReferencedActor)
                continue;
            FString PropertyName = PrefabProperty->PropertyName;
            if (PropertyName == "AssetUserData")
                continue; // Skip this as assignment is very slow and is not needed
            ...
            if (Property)
            {
                ...
                if (FObjectProperty* ObjProperty = CastField<FObjectProperty>(Property))
                {
                    UObject* PropertyObjectValue =
                      ObjProperty->GetObjectPropertyValue_InContainer(InObjToDeserialize);
                    if (PropertyObjectValue &&
                        PropertyObjectValue->HasAnyFlags(RF_DefaultSubObject | RF_ArchetypeObject))
                    {
                        continue;
                    }
                }
...

                {
                    ...
                    PropertyPathHelpers::SetPropertyValueFromString(
                      InObjToDeserialize, PrefabProperty->PropertyName, PrefabProperty->ExportedValue);
                }
            }
        }
    }

    void SerializeFields(UObject*                         ObjToSerialize,
                         UObject*                         ObjTemplate,
                         APrefabActor*                    PrefabActor,
                         const FPrefabActorLookup&        CrossReferences,
                         TArray<UPrefabricatorProperty*>& OutProperties)
    {
        ...
        for (TFieldIterator<FProperty> PropertyIterator(ObjToSerialize->GetClass()); PropertyIterator;
             ++PropertyIterator)
        {
            ...
            {
                UObject* PropertyObjectValue =
                  ObjProperty->GetObjectPropertyValue_InContainer(ObjToSerialize);
                if (PropertyObjectValue &&
                    PropertyObjectValue->HasAnyFlags(RF_DefaultSubObject | RF_ArchetypeObject))
                {
                    continue;
    ...
    void CollectAllSubobjects(UObject* Object, TArray<UObject*>& OutSubobjectArray)
    {
        ...
        for (int32 ComponentIndex = 0; ComponentIndex < OutSubobjectArray.Num(); ComponentIndex++)
        {
            ...
            if (!PotentialComponent->IsDefaultSubobject() &&
                !PotentialComponent->HasAnyFlags(RF_DefaultSubObject))
            {
    ...
    void DumpSerializedData(const FPrefabricatorActorData& InActorData)
    {
        ...
        UE_LOG(
          LogPrefabTools, Log, TEXT("Actor Properties: %s"), *InActorData.ClassPathRef.GetAssetPathString());
        UE_LOG(LogPrefabTools, Log, TEXT("================="));
...
{
    static const TSet<FName> IgnoredFields = { "AttachParent",          "AttachSocketName",
                                               "AttachChildren",        "ClientAttachedChildren",
                                               "bIsEditorPreviewActor", "bIsEditorOnlyActor",
                                               "UCSModifiedProperties", "BlueprintCreatedComponents" };
...
{
    static const TSet<FName> FieldsToForceSerialize = { "Mobility" };
...
namespace
{
    UActorComponent* FindBestComponentInCDO(AActor* CDO, UActorComponent* Component)
    {
        ...
        {
            if (DefaultComponent && DefaultComponent->GetFName() == Component->GetFName() &&
...
void FPrefabTools::SaveActorState(AActor*                   InActor,
                                  APrefabActor*             PrefabActor,
                                  const FPrefabActorLookup& CrossReferences,
...
void FPrefabTools::LoadActorState(AActor*                        InActor,
                                  const FPrefabricatorActorData& InActorData,
                                  const FPrefabLoadSettings&     InSettings)
{
...
namespace
{
    void GetPrefabBoundsRecursive(AActor*              InActor,
                                  FBox&                OutBounds,
                                  bool                 bNonColliding,
                                  const TSet<UClass*>& IgnoreActorClasses)
    {
        if (InActor && InActor->IsLevelBoundsRelevant())
        {
            const bool bIgnoreBounds =
              InActor->IsA<APrefabActor>() || IgnoreActorClasses.Contains(InActor->GetClass());
            if (!bIgnoreBounds)
            {
...
void FPrefabTools::LoadStateFromPrefabAsset(APrefabActor* PrefabActor, const FPrefabLoadSettings& InSettings)
{
    ...
    for (AActor* ExistingActor : ExistingActorPool)
    {
        ...
        {
            UPrefabricatorAssetUserData* PrefabUserData =
              ExistingActor->GetRootComponent()->GetAssetUserData<UPrefabricatorAssetUserData>();
    ...
    if (Service.IsValid())
    {
        ...
        for (FPrefabricatorActorData& ActorItemData : PrefabAsset->ActorData)
        {
            ...
            UClass* ActorClass =
              LoadObject<UClass>(nullptr, *ActorItemData.ClassPathRef.GetAssetPathString());
            if (!ActorClass)
                continue;
            ...
            if (!ChildActor)
            {
                // Create a new child actor.  Try to create it from an existing template actor that is already
                // preset in the scene
                ...
                ChildActor =
                  Service->SpawnActor(ActorClass, WorldTransform, PrefabActor->GetLevel(), Template);
                SpawnedActors.Add(ChildActor);
                ...
                if (Template == nullptr || bPrefabOutOfDate)
                {
                    ...
                    {
                        LoadState->RegisterTemplate(
                          ActorItemData.PrefabItemID, PrefabAsset->LastUpdateID, ChildActor);
                    }
                }
            }
            else
            {
                ...
                // Update the world transform.   The reuse happens only on leaf actors (which don't have any
                // further child actors)
        ...
        for (int i = 0; i < PrefabAsset->ActorData.Num(); ++i)
        {
            const FPrefabricatorActorData& ActorItemData = PrefabAsset->ActorData[i];
            if (ActorItemData.ParentIndex > -1)
            {
                SpawnedActors[i]->DetachFromActor(FDetachmentTransformRules::KeepWorldTransform);
                SpawnedActors[i]->AttachToActor(SpawnedActors[ActorItemData.ParentIndex],
...
void FPrefabTools::FixupCrossReferences(const TArray<UPrefabricatorProperty*>& PrefabProperties,
                                        UObject*                               ObjToWrite,
                                        TMap<FGuid, AActor*>&                  PrefabItemToActorMap)
{
    for (UPrefabricatorProperty* PrefabProperty : PrefabProperties)
    {
        ...
        UE_LOG(LogPrefabTools,
               Log,
               TEXT("Cross Reference: %s -> %s"),
               *PrefabProperty->CrossReferencePrefabActorId.ToString(),
               *ActorName);
        ////////
    }
}
...
void FPrefabInstanceTemplates::RegisterTemplate(const FGuid& InPrefabItemId,
                                                FGuid        InPrefabLastUpdateId,
                                                AActor*      InActor)
{
